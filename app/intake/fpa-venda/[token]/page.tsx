...
export default function FpaVendaIntakePage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/investor/fpa-venda-intake/${token}`)
      .then(async (res) => {
    ...
    <div className="min-h-screen bg-[#09081A]">
      <div className="border-b border-[#C9A84C]/20 bg-[#12112A]">
        <div className="max-w-2xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-[#F5F1E8]">FPA Venda, Acordo de Proteção de Honorários</p>
            <p className="text-[10px] text-[#9BAFC5]">Cadastro do grupo de venda, V3 Partners</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {state === "locked" && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
            ...
            <h2 className="text-xl font-bold text-[#F5F1E8] mb-2">{signed ? "FPA Venda assinada" : "Aguardando assinatura"}</h2>
        ...
        {state === "ready" && data && (
          <FpaIntakeForm token={token} apiPath="fpa-venda-intake" dealCode={data.deal_code} withValor withSignature deducaoPercent={data.deducao_percent} />
        )}
      </div>
