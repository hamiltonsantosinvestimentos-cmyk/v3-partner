...
import { LoiIntakeForm } from "@/components/investor/loi-intake-form";

export default function CartaIntencaoIntakePage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/investor/loi-intake/${token}`)
      .then(async (res) => {
    ...
    <div className="min-h-screen bg-[#09081A]">
      <div className="border-b border-[#C9A84C]/20 bg-[#12112A]">
        <div className="max-w-2xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-[#F5F1E8]">Carta de Intenção de Compra</p>
            <p className="text-[10px] text-[#9BAFC5]">Qualificação de comprador, V3 Partners</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {state === "locked" && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
            ...
            <h2 className="text-xl font-bold text-[#F5F1E8] mb-2">{signed ? "Carta de Intenção assinada" : "Aguardando assinatura"}</h2>
            <p className="text-sm text-[#9BAFC5] max-w-md">{errorMsg}</p>
            <p className="text-xs text-[#9BAFC5]/50 mt-6">Entre em contato: deal@v3partners.com.br</p>
          </div>
        )}

        {state === "ready" && data && <LoiIntakeForm token={token} context={data} />}
      </div>
