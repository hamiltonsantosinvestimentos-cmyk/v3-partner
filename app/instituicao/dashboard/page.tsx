...
// ── Kanban Card ──────────────────────────────────────────────────────────────
function KanbanCard({ p, onClick }: { p: Proposta; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="bg-[#111F35] border border-[#243A66] hover:border-[#C9A84C]/40 rounded-xl p-4 cursor-pointer transition-all hover:bg-[#162744]/60 space-y-3"
    >
      <div className="flex items-start justify-between gap-2">
        <span className="text-[#C9A84C] font-mono text-[10px] font-bold">{p.code}</span>
        <span className="text-[9px] text-[#7A8FA8]">{formatDate(p.updated_at)}</span>
      </div>
      <div>
        <p className="text-xs font-semibold text-[#F0ECE4] leading-snug">{p.client_name}</p>
        <p className="text-[10px] text-[#7A8FA8] mt-0.5">{p.credit_line}</p>
      </div>
      {p.pending_reason && (
        <div className="flex items-start gap-1.5 bg-amber-500/10 border border-amber-500/20 rounded-lg px-2.5 py-2">
          ...
          <p className="text-[10px] text-amber-300 leading-snug">
            {p.pending_reason.length > 70 ? p.pending_reason.slice(0, 70) + "…" : p.pending_reason}
          </p>
        </div>
      )}
      <div className="flex items-center justify-between pt-1 border-t border-[#243A66]/40">
        <span className="text-xs font-bold text-[#F0ECE4]">{formatBRL(p.requested_value)}</span>
        {p.approved_value && (
          <span className="text-[10px] text-emerald-400 font-semibold">✓ {formatBRL(p.approved_value)}</span>
        )}
      </div>
    </div>
  );
...
function KanbanColumn({
  title, icon, color, propostas, onCardClick,
}: {
  title: string;
  ...
  const total = propostas.reduce((a, p) => a + p.requested_value, 0);
  return (
    <div className="flex-1 min-w-0">
      <div className={`flex items-center gap-2 mb-3 px-1`}>
        {icon}
        <span className={`text-xs font-bold ${color}`}>{title}</span>
        <span className="ml-auto text-[10px] font-bold bg-[#162744] border border-[#243A66] px-2 py-0.5 rounded-full text-[#7A8FA8]">
          {propostas.length}
        </span>
      </div>
      <div className="text-[10px] text-[#7A8FA8] px-1 mb-3">
        Volume: <span className="text-[#C9A84C] font-semibold">{formatBRL(total)}</span>
      </div>
      <div className="space-y-3 min-h-[120px]">
        {propostas.length === 0 ? (
          <div className="border-2 border-dashed border-[#243A66]/40 rounded-xl py-10 text-center">
            <p className="text-xs text-[#243A66]">Nenhuma proposta</p>
          </div>
        ) : (
          propostas.map(p => (
            <KanbanCard key={p.id} p={p} onClick={() => onCardClick(p.id)} />
          ))
        )}
      </div>
    </div>
...
export default function InstituicaoDashboard() {
  ...
  useEffect(() => {
    ...
    const inst_id = sessionStorage.getItem("instituicao_id") ?? "";
    const inst_email = sessionStorage.getItem("instituicao_email") ?? "";
    if (!inst_nome) { window.location.href = "/instituicao/login"; return; }
    setNome(inst_nome);
    setInstId(inst_id);
    void inst_email;
  }, []);
  ...
  useEffect(() => { if (nome) fetchPropostas(); }, [nome, fetchPropostas]);
  ...
  const emAnalise = filtered.filter(p => p.stage === "ANALISE");
  const emPendencia = filtered.filter(p => p.stage === "PENDENCIA");
    ...
    <div className="min-h-screen bg-[#09081A]">
      ...
      <header className="border-b border-[#243A66] bg-[#09081A]/95 sticky top-0 z-10 backdrop-blur">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              href="/instituicao/configuracoes"
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center bg-[#111F35] border border-[#243A66] rounded-xl p-1 gap-1">
            <button
              onClick={() => setView("kanban")}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${view === "kanban" ? "bg-[#C9A84C] text-[#09081A]" : "text-[#7A8FA8] hover:text-[#F0ECE4]"}`}
            >
              <LayoutGrid className="w-3.5 h-3.5" /> Kanban
            </button>
            <button
              onClick={() => setView("list")}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${view === "list" ? "bg-[#C9A84C] text-[#09081A]" : "text-[#7A8FA8] hover:text-[#F0ECE4]"}`}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="bg-[#111F35] border border-[#243A66] rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-[#F0ECE4]">{propostas.filter(p => p.stage === "ANALISE").length}</div>
            <div className="text-[10px] text-[#7A8FA8] mt-1">proposta{propostas.filter(p => p.stage === "ANALISE").length !== 1 ? "s" : ""}</div>
          </div>
          <div className="bg-[#111F35] border border-[#243A66] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <div className="text-2xl font-bold text-[#F0ECE4]">{propostas.filter(p => p.stage === "PENDENCIA").length}</div>
          ...
          <div className="bg-[#111F35] border border-[#243A66] rounded-2xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#243A66]/40">
                {filtered.map(p => (
                  <tr key={p.id} onClick={() => setSelectedId(p.id)} className="hover:bg-[#162744]/40 transition-colors cursor-pointer">
                    ...
                    <td className="px-4 py-3.5 hidden sm:table-cell">
                      ...
                      {p.approved_value && (
                        <div className="text-[10px] text-emerald-400 mt-0.5">Aprovado: {formatBRL(p.approved_value)}</div>
                      )}
                    </td>
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${STAGE_COLORS[p.stage] ?? "bg-[#243A66]/30 text-[#7A8FA8] border-[#243A66]"}`}>
                        ...
                        {p.stage === "ANALISE" ? "Em Análise" : "Pendência"}
                      </span>
                    </td>
