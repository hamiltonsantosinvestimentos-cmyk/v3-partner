...
export async function POST(req: NextRequest) {
  // P0 real, mesmo padrão achado e corrigido em contract-upload-extract.ts
  // (02/09/2026): esta rota inteira não tinha NENHUM try/catch. Qualquer
  // exceção em qualquer um dos passos (import de mammoth, geração de PDF,
  // merge/estampa, upload duplo pro Storage) virava página HTML genérica
  // de erro do Vercel, não JSON -- exatamente o sintoma que escondeu a
  // causa raiz do bug de upload de minuta. Handler inteiro agora dentro de
  // um único try/catch, com console.error do erro real.
  try {
    ...
    if (ext === ".pdf") {
      ...
      // Import dinâmico (02/09/2026, mesma correção de contract-upload-extract.ts):
      // import estático de mammoth no topo do módulo já derrubou esta rota
      // inteira no cold start, para QUALQUER extensão, não só .docx.
    ...
    console.error("[contracts/manual-intake] falha não tratada:", err);
    return NextResponse.json({ error: `Erro ao regularizar contrato manual: ${err?.message ?? "erro desconhecido"}` }, { status: 500 });
  }
}
