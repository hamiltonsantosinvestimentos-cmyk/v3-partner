...
// GET /api/sdr/campanhas-whatsapp/[id] — detalhe da campanha + fila de contatos
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: campanha }, { data: contatos }] = await Promise.all([
    db.from("sdr_campanhas_whatsapp").select("*").eq("id", id).single(),
...
// PATCH /api/sdr/campanhas-whatsapp/[id] — atualiza nome/mensagem/intervalo/status (nunca dispara envio)
...
// DELETE /api/sdr/campanhas-whatsapp/[id] — remove a campanha e seus contatos (ADMIN)
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await svc().from("sdr_campanhas_whatsapp").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
