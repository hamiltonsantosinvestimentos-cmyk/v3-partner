...
// POST — gera novo link M&A (autenticado, PARTNER / PARTNER_PRO / ADMIN)
export async function POST() {
  ...
  if (!["PARTNER", "PARTNER_PRO", "ADMIN"].includes(profile?.role ?? "")) {
    return NextResponse.json({ error: "Apenas Partners podem gerar links M&A" }, { status: 403 });
  }

  const token = randomBytes(20).toString("hex");
  const svc = serviceClient();

  const { data, error } = await svc.from("ma_captacao_links").insert({
    token,
    partner_id: user.id,
...
export async function PATCH(req: NextRequest) {
  ...
  const svc = serviceClient();
  const { error } = await svc.from("ma_captacao_links").update({ active }).eq("id", id).eq("partner_id", user.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
