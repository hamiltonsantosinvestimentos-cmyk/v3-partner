-- Loop 2 (redirecionado hoje): Central de Contratos pronta para enviar.
-- Passo 1: bootstrap do vertical Contratos (5 subpastas) na governanca MPS.

alter table folder_registry drop constraint if exists folder_registry_vertical_check;
alter table folder_registry add constraint folder_registry_vertical_check check (vertical in ('MA','Credito','Consorcios','BolsaDeAtivos','Administracao','Contratos'));
...
insert into folder_registry (vertical, full_path, depth, status, created_by)
select 'Contratos', v.full_path, 1, 'active', (select id from profiles where role='ADMIN' limit 1)
from (values
  ('Contratos/Contratos_de_Partners'),
  ('Contratos/Contratos_com_Fundos'),
  ('Contratos/Contratos_de_Fornecedores'),
  ('Contratos/Contratos_de_Contabilidade'),
  ('Contratos/Contratos_de_Securitizadora')
) as v(full_path)
where not exists (select 1 from folder_registry fr where fr.full_path = v.full_path);

-- Passo 3 (parcial, os 2 contratos reais de hoje): templates + operation_contracts
-- ja prontos, texto juridico literal fornecido por Joao, so formatado no wrapper V3.

with tpl_closer as (
  insert into contract_templates (template_name, vertical, contract_series, body_text_raw, is_active, created_by)
  values (
    'Contrato de Parceria Comercial, Closer & Partner PRO, Iris Rodrigues da Silva',
    'clientes', 'V3C-PAR',
    $html$<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
code_closer as (select next_v3_code('V3C-PAR', null) as code)
insert into operation_contracts (template_id, contract_code, vertical, contract_title, rendered_html, status_signature, parties, created_by)
select tpl_closer.id, code_closer.code, 'clientes',
  'Contrato de Parceria Comercial, Closer & Partner PRO, Iris Rodrigues da Silva',
  $html$<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<p>Este documento requer assinatura eletrônica para validade jurídica.</p>
  ...
  'rascunho', '[{"role":"closer","name":"Iris Rodrigues da Silva","doc":"CNPJ 15.133.730/0001-38 · CPF 100.040.226-61","email":"iriswickman@hotmail.com"},{"role":"v3_partners","name":"V3 Partners Soluções Ltda","doc":"14.219.287/0001-50"}]'::jsonb,
  (select id from profiles where role='ADMIN' limit 1)
from tpl_closer, code_closer;

with tpl_hc as (
  insert into contract_templates (template_name, vertical, contract_series, body_text_raw, is_active, created_by)
  values (
    'Contrato de Parceria Comercial e Intermediacao de Negocios, Home Cash',
    'clientes', 'V3C-PAR',
    $html$<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
code_hc as (select next_v3_code('V3C-PAR', null) as code)
insert into operation_contracts (template_id, contract_code, vertical, contract_title, rendered_html, status_signature, parties, created_by)
select tpl_hc.id, code_hc.code, 'clientes',
  'Contrato de Parceria Comercial e Intermediacao de Negocios, Home Cash',
  $html$<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<p>Este documento requer assinatura eletrônica para validade jurídica.</p>
  ...
  'rascunho', '[{"role":"estruturador_1","name":"Daniel Alves Nicolau","doc":"CPF 371.203.358-39","email":"dn@ssnpar.com"},{"role":"estruturador_2","name":"Diogo Galindo Marchesano","doc":"CPF 427.126.278-14","email":"diogo.marchesano@dgmconsulting.com.br"},{"role":"v3_partners","name":"V3 Partners Soluções Ltda","doc":"14.219.287/0001-50"}]'::jsonb,
  (select id from profiles where role='ADMIN' limit 1)
from tpl_hc, code_hc;

select contract_code, contract_title, vertical, status_signature, jsonb_array_length(parties) as n_parties
from operation_contracts where contract_code like 'V3C-PAR%' or template_id in (
  select id from contract_templates where template_name ilike '%Iris%' or template_name ilike '%Home Cash%'
